<?php $pageName = 'ค้นหารายการขอลดหุ้น'; ?>
...
<section class="content-header">
    ...
    <a href="{{url('/stkdown')}}" class="btn btn-app btn-app-add pull-right ">
            ...
            <div class="box-body">
                ...
                <table class="table table-bordered table-hover" id="cusdatatable"  style="width: 100%" >
                    ...
                    <tbody >
                        ...
                        <tr style="" data-cus="" >
                            <td style="text-align: center" >{{$res->XVDowDocNo}}</td>  
                            <td style="text-align: center" >{{$res->XDDowDocDate}}</td>  
                            <td>{{$res->XVCusCode}}</td>
                            <td>{{$res->XVCusName}}</td>
                            <td>{{$res->XVBchName}}</td>
                            <td>{{$res->XVDivName}}</td>                            
                            <td style="text-align: center"><?php
                            switch ($res->XVDowDocStatus){
                                case '1':case '':
                                    if ($res->XBDowDocSend == true){
                            ...
                            <a href="{{ url('/stkdown/'.$res->XVDowDocNo)}}" class="btn btn-warning btn-xs "><i class="fa fa-pencil"></i> จัดการ </a>
                        </td>                            
                        </tr>
                        @endforeach
                    </tbody>
...
$('#cus_code').keyup(function(e){
    if(e.keyCode == 13)
    {
        ...
        if ($('#cus_code').val().length > 0 ){
            url = "{{URL::to('/stkdown_main/')}}"+'/'+$('#cus_code').val();
